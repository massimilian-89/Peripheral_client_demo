...
static uint8_t ADV_status_flag = 0;
static uint8_t ccc_value = 0x0001;                      // Enable notifications (0x0002 for indications)
static bool switchPressed = false;              
static bool   uuid_matched = false;
					...
					case GATTC_READ_IND:
					 ...
					 case GATTC_EVENT_IND:
						 
								{
										...
										arch_printf("\n\n\rnotification received...");

										arch_printf("\n\n\rvalue: ");
										for (uint16_t i = 0; i < notify_ind->length; i++) {
												arch_printf("0x%02X ", notify_ind->value[i]);
										}
